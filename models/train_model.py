...
from models.model import GiraffeDecoderV1, GiraffeDecoderV2, GiraffeDecoderV3, GiraffeDecoderV4, GiraffeDecoderV5, GiraffeDecoderV6, KmeansSlotAttention, ShpAppEncoderV1, ShpAppEncoderV2, Decoder, ShpAppDecoderV1, ShpAppDecoderV2, Discriminator, Encoder, SlotAttention, d_logistic_loss, d_r1_loss, g_nonsaturating_loss, get_perceptual_net, raw2outputs, toggle_grad
...
class uorfGanModel(pl.LightningModule):
    ...
    def forward(self, input):
        ...
        # Run encoder on first images, then flatten H×W to F;  Eine Szene besteht aus 4 Bilder und feature_map läuft nur auf erstem Bild.ö
        ...
        """
        Idee:
        z_slots[:, :, 0:32] --> z_shape
        z_slots[:, :, 32:] --> z_appearance
        """
        ...
        sampling_coor_fg = frus_nss_coor[:, None, ...].expand(-1, K - 1, -1, -1)  # B×(K-1)xPx3  # -1 bedeutet Dimensionsgröße übernehmen
        ...
        rgb_maps = []
        for i in range(B):
            rgb_map, _, _ = raw2outputs(raws[i], z_vals[i], ray_dir[i]) #[N_rays, 3] [16384, 3]
...
class KmeansuorfGanModel(pl.LightningModule):
    ...
    def __init__(self, opt):
        ...
        # Initialize slot attention
        self.netSlotAttention = KmeansSlotAttention(
            num_slots=opt.num_slots, in_dim=z_dim, slot_dim=z_dim, iters=opt.attn_iter)
        init_weights(self.netSlotAttention, init_type='normal', init_gain=0.02)
    ...
    def forward(self, input):
        ...
        # Repeat sampling coordinates for each object (K-1 objects, one background), P = S*D*H*W
        sampling_coor_fg = frus_nss_coor[:, None, ...].expand(-1, K - 1, -1,
                                                              -1)  # B×(K-1)xPx3  # -1 bedeutet Dimensionsgröße übernehmen
        sampling_coor_bg = frus_nss_coor  # B×Px3
        ...
        W, H, D = self.opt.supervision_size, self.opt.supervision_size, self.opt.n_samp
        imgs = imgs.view(B, S, C, H, W)
        raws = raws.view([B, N, D, H, W, 4]).permute([0, 1, 3, 4, 2, 5]).flatten(start_dim=1,
                                                                                 end_dim=3)  # B×(NxHxW)xDx4
        masked_raws = masked_raws.view([B, K, N, D, H, W, 4])
        unmasked_raws = unmasked_raws.view([B, K, N, D, H, W, 4])
...
class ShapeAppV1DV1UorfGanModel(pl.LightningModule):
    ...
    def __init__(self, opt):
        ...
        # Initialize ShpAppEnc
        self.netShpAppEnc = ShpAppEncoderV1(z_dim=z_dim)
        init_weights(self.netShpAppEnc, init_type='xavier', init_gain=0.02)

        # Initialize nerf decoder network
        self.netDecoder = ShpAppDecoderV1(
            n_freq=opt.n_freq, input_dim=6 * opt.n_freq + 3 + z_dim, z_dim=opt.z_dim,
            n_layers=opt.n_layer, locality_ratio=opt.obj_scale / opt.nss_scale,
            fixed_locality=opt.fixed_locality)
        init_weights(self.netDecoder, init_type='xavier', init_gain=0.02)
    ...
    def forward(self, input):
        ...
        # Run ShpAppEncoder
        z_shp_fg, z_app_fg, z_shp_bg, z_app_bg = self.netShpAppEnc(z_slots[0, 1:, :], z_slots[0, 0:1, :])
        z_shp_fg, z_app_fg, z_shp_bg, z_app_bg = z_shp_fg.unsqueeze(0), z_app_fg.unsqueeze(0), z_shp_bg.unsqueeze(0), z_app_bg.unsqueeze(0)
        # Run decoder
        raws, masked_raws, unmasked_raws, maks = self.netDecoder(
            sampling_coor_bg, sampling_coor_fg, z_shp_fg, z_app_fg, z_shp_bg, z_app_bg,  nss2cam0)
        # (NxDxHxW)x4, Kx(NxDxHxW)x4, Kx(NxDxHxW)x4,  (Kx(NxDxHxW)x1 <- masks, not needed)
    ...
    def log_visualizations(self, imgs: torch.Tensor, imgs_reconstructed: torch.Tensor, raw_data) -> None:
        ...
        if self.trainer.is_global_zero and isinstance(self.logger, TensorBoardLogger):
            ...
            avg_grad_plot = make_average_gradient_plot(chain(
                self.netEncoder.named_parameters(),
                self.netSlotAttention.named_parameters(),
                self.netShpAppEnc.named_parameters(),
                self.netDecoder.named_parameters())
            )
...
class ShapeAppV2DV1UorfGanModel(pl.LightningModule):
    ...
    def __init__(self, opt):
        ...
        # Initialize ShpAppEnc
        self.netShpAppEnc = ShpAppEncoderV2(z_dim=z_dim)
        init_weights(self.netShpAppEnc, init_type='xavier', init_gain=0.02)
...
class ShapeAppV1DV2UorfGanModel(pl.LightningModule):
    ...
    def __init__(self, opt):
        ...
        # Initialize nerf decoder network
        self.netDecoder = ShpAppDecoderV2(
            n_freq=opt.n_freq, input_dim=6 * opt.n_freq + 3 + z_dim, z_dim=opt.z_dim,
            n_layers=opt.n_layer, locality_ratio=opt.obj_scale / opt.nss_scale,
            fixed_locality=opt.fixed_locality)
        init_weights(self.netDecoder, init_type='xavier', init_gain=0.02)
...
class ShapeAppV2DV2UorfGanModel(pl.LightningModule):
...
class Decv1uorfGanModel(pl.LightningModule):
    ...
    def __init__(self, opt):
        ...
        # Initialize nerf decoder network
        self.netDecoder = GiraffeDecoderV1(
            n_freq=opt.n_freq, input_dim=6 * opt.n_freq + 3 + z_dim, z_dim=opt.z_dim,
            n_layers=opt.n_layer, locality_ratio=opt.obj_scale / opt.nss_scale,
            fixed_locality=opt.fixed_locality)
        init_weights(self.netDecoder, init_type='xavier', init_gain=0.02)
...
class Decv2uorfGanModel(pl.LightningModule):
    ...
    def __init__(self, opt):
        ...
        # Initialize nerf decoder network
        self.netDecoder = GiraffeDecoderV2(
            n_freq=opt.n_freq, input_dim=6 * opt.n_freq + 3 + z_dim, z_dim=opt.z_dim,
            n_layers=opt.n_layer, locality_ratio=opt.obj_scale / opt.nss_scale,
            fixed_locality=opt.fixed_locality)
        init_weights(self.netDecoder, init_type='xavier', init_gain=0.02)
...
class Decv3uorfGanModel(pl.LightningModule):
    ...
    def __init__(self, opt):
        ...
        # Initialize nerf decoder network
        self.netDecoder = GiraffeDecoderV3(
            n_freq=opt.n_freq, input_dim=6 * opt.n_freq + 3 + z_dim, z_dim=opt.z_dim,
            n_layers=opt.n_layer, locality_ratio=opt.obj_scale / opt.nss_scale,
            fixed_locality=opt.fixed_locality)
        init_weights(self.netDecoder, init_type='xavier', init_gain=0.02)
...
class Decv4uorfGanModel(pl.LightningModule):
    ...
    def __init__(self, opt):
        ...
        # Initialize nerf decoder network
        self.netDecoder = GiraffeDecoderV4(
            n_freq=opt.n_freq, input_dim=6 * opt.n_freq + 3 + z_dim, z_dim=opt.z_dim,
            n_layers=opt.n_layer, locality_ratio=opt.obj_scale / opt.nss_scale,
            fixed_locality=opt.fixed_locality)
        init_weights(self.netDecoder, init_type='xavier', init_gain=0.02)
...
class Decv5uorfGanModel(pl.LightningModule):
    ...
    def __init__(self, opt):
        ...
        # Initialize nerf decoder network
        self.netDecoder = GiraffeDecoderV5(
            n_freq=opt.n_freq, input_dim=6 * opt.n_freq + 3 + z_dim, z_dim=opt.z_dim,
            n_layers=opt.n_layer, locality_ratio=opt.obj_scale / opt.nss_scale,
            fixed_locality=opt.fixed_locality)
        init_weights(self.netDecoder, init_type='xavier', init_gain=0.02)
...
class Decv6uorfGanModel(pl.LightningModule):
    ...
    def __init__(self, opt):
        ...
        # Initialize nerf decoder network
        self.netDecoder = GiraffeDecoderV6(
            n_freq=opt.n_freq, input_dim=6 * opt.n_freq + 3 + z_dim, z_dim=opt.z_dim,
            n_layers=opt.n_layer, locality_ratio=opt.obj_scale / opt.nss_scale,
            fixed_locality=opt.fixed_locality)
        init_weights(self.netDecoder, init_type='xavier', init_gain=0.02)
